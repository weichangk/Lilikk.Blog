...
- 启动容器
    ...
    docker run -d -p 8001:8001 --restart=always -v /home/blog.api/wwwroot:/app/wwwroot  --name blog.api blog.api
    ```
- 查看容器 `docker ps -a`， 验证 `curl 127.0.0.1:8001/index.html`
...
if [ -d "lilikk.blog" ];then
    ...
    echo "拉取代码..."
    git clone  https://github.com/weichangk/Lilikk.Blog.git lilikk.blog
    cd lilikk.blog
fi

echo "拷贝生产配置"
cp -f ../blog.api.config/applicationsettings.json ./src/backend/Blog.Admin.Application/

echo "获取当前容器是否存在"
containerps=$(docker ps -f name=blog.api -q)
containerstop=$(docker ps -a -f name=blog.api -q)
...
echo "获取当前镜像是否存在-----------------------------------------------------------------"
dockerlist=$(docker images blog.api:latest -q)
...
#docker run -d -p 8001:8001 -v /home/blog.api/wwwroot:/app/wwwroot --restart=always --name blog.api blog.api
```
